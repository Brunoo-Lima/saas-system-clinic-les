...
export class ClinicFactory {
  static createFromDTO(clinicDTO: ClinicDTO) {
    const insurances = clinicDTO.insurances?.map((ins) => {
    ...
    .build()
        ...
        // Country
        const country = new Country({ name: clinicDTO.address?.country.name ?? "" });
        country.setUuidHash(
          clinicDTO.address?.country.id ?? country.getUUIDHash()
        );
    ...
    const clinic = new ClinicBuilder()
      ...
      .setTimeToConfirm(clinicDTO.timeToConfirm ?? 86.400) //  86.400 = 24 Horas
      .setAddress(address)
