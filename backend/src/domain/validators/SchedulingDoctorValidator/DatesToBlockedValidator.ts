...
export class DatesToBlockedValidator implements IProcessValidator{
    async valid(datesToBlocked: Array<SchedulingBlockedDays>){
        try {
            ...
            if(!timesFiltered.length) return ResponseHandler.error("Nao existe datas validas para bloquear")
            const datesNotDuplicates = new Set(timesFiltered) 
            if(datesNotDuplicates.size !== timesFiltered.length) return ResponseHandler.error("Não pode inserir datas duplicadas")
