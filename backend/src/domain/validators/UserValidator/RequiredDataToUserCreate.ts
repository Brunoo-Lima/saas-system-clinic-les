...
export class RequiredDataToUserCreate implements IProcessValidator{
    valid(user: User){
        const objectResponse = { success: true, message: "All required data is present."};
        const keysRequired = ['email', 'password', 'passwordConfirmed', 'role'];
        ...
        return ResponseHandler.success(objectResponse, "All required data is present.");
    }
}